...
import hu.bme.aut.qrvhfq.EnchantedEmporium.EEAplication
        ...
        viewModelScope.launch {
            try {
                val imageBitmap = MediaStore.Images.Media.getBitmap(
                    getApplication<EEAplication>().contentResolver,
                    imageUri
                )
                ...
                saveUserInformation(user.copy(imagePatg = imageUrl), false)
            } catch (e: Exception) {
                viewModelScope.launch {
    ...
    private fun saveUserInformation(user: User, shouldRetrievedOldImage: Boolean) {
        firestore.runTransaction { transaction ->
            ...
            if (shouldRetrievedOldImage) {
                ...
                val newUser = user.copy(imagePatg = currentUser?.imagePatg ?: "")
